...
## Things the score does NOT account for

Treat as caveats when reading any individual row:

- **Pre-existing vendor relationships** — we infer where possible (`known_synthesis_vendors`) but the model often guesses; treat that field as soft signal, not ground truth.
- **Current procurement freeze / hiring freeze** — not visible from public sources.
- **Founder network access** — a Tier 3 company may be a hot lead if your CEO went to grad school with theirs. The score is naive to your team's existing relationships.
- **Geographic considerations** — `outreach_difficulty` doesn't model time zones, country-specific procurement rules, or "do we have a local rep."
- **Bench-headcount estimate confidence** — for companies smaller than ~50 employees, we can usually only estimate. The `bench_headcount_estimate.confidence` field tells you how confident the model was; treat `low` as "this is a wet guess."
- **Surface-mode shallowness** — these scores were generated from 5 web searches. A few names got it wrong (e.g. funding sums that double-count earlier rounds). The Deep Research equivalents in `runs/{ts}_{slug}/brief.json` are more authoritative when they exist.

---

## How to re-rank with different priorities

`build_targets.py` parameterizes the score cutoff:

```bash
# Only Tier 1 + Tier 2
python build_targets.py --min-score 4

# Loosen to include Tier 3 — default
python build_targets.py --min-score 3

# Generate per-company briefs for all 255, regardless of score
python build_targets.py --include-all
```

If you want to change the rubric itself (e.g. weight bench headcount more heavily, deprioritize public companies, or score by region), the rubric lives in the system prompt at `test_query.py:DEVELOPER_MESSAGE` + the user message template. Re-running scoring requires re-running the Claude surface runner with the new prompt — about 15 min and ~$25 at current rates.
